...
class OfflineReadingScreen extends Component {
  ...
  savePatients = newPatients => {
    const savePatients = AsyncStorage.setItem(
      'patients',
  ...
  readDataFromBluetooth() {
    ...
    BluetoothSerial.readFromDevice().then(data => {
      ...
      console.log(data[length - 1].charCodeAt(0) + ' <this is data');
      var tmp = data[length - 1].trim().charCodeAt(0).toString();
      var lable = data[length - 2].trim().charCodeAt(0).toString();
      console.log(tmp, lable);
      if (tmp === '') {
        Alert.alert(
          'Alert',
          'No data found!',
