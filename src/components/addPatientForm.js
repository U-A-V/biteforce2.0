...
const AddPatientSchema = yup.object({
    ...
    Age: yup.string().required().max(3, 'Age must be at most 3 digits').test('is-valid-age', 'Age must be less than 120', (val) => {
        return parseInt(val) <= 120;
    }),
    Email: yup.string().required().email(),
    Phone: yup.string().required('Phone number is a required field').length(10, 'Phone number must be exactly 10 digits'),
})
                ...
                {(props) => (
                    <View>
                        ...
                        <View style={{marginVertical: 40}}>
                            <Button raised title='Add Patient' onPress={() => {
                                props.handleSubmit();
